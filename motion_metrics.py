...
class MetricsEngine(object):
    ...
    def create_summaries(self):
        """
        Create placeholders and summary ops for the joint angle metric and target length that we want to evaluate.
        """
    ...
    def get_summary_feed_dict(self, final_metrics):
        """
        Compute the joint angle metric for the target sequence lengths and return the feed dict that can be used in a
        call to `sess.run` to retrieve the Tensorboard summary ops.
        Args:
            final_metrics: Dictionary of metric values, expects them to be in shape (seq_length, ).
    ...
    def compute(self, predictions, targets, reduce_fn="mean"):
        ...
        # add missing joints
        pred = sparse_to_full(pred, SMPL_MAJOR_JOINTS, SMPL_NR_JOINTS)
        targ = sparse_to_full(targ, SMPL_MAJOR_JOINTS, SMPL_NR_JOINTS)
        ...
        pred_global = local_rot_to_global(pred, SMPL_PARENTS, left_mult=False)  # (-1, full_n_joints, 3, 3)
        targ_global = local_rot_to_global(targ, SMPL_PARENTS, left_mult=False)  # (-1, full_n_joints, 3, 3)
